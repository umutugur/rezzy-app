...
export default function NotificationsScreen({ navigation }: any) {
 const {
  ...
  markAllReadLocal,
  markAllReadRemote,
  markReadLocal,       // 👈 store’dan al
  markReadRemote,      // 👈 opsiyonel
} = useNotifications();

  useEffect(() => {
    // server’dan liste & sayaç (uç yoksa sessiz)
    fetchListRemote?.();
    fetchUnreadCount?.();
  }, []);
  ...
  const handleOpen = async (id: string, data: any) => {
  try {
    if (typeof markReadRemote === "function") {
    ...
    try {
      ...
      Alert.alert("Bildirim", "İlgili sayfa açılamadı.");
    }
  }
};
  const renderItem = ({ item, index }: { item: any; index: number }) => (
    <TouchableOpacity
      activeOpacity={0.9}
      onPress={() => handleOpen(String(item.id), item.data)}
      style={{
        ...
        ListHeaderComponent={
          <View style={{ padding: 16, paddingBottom: 8, backgroundColor: "#fff" }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <TouchableOpacity
                onPress={async () => {
                  if (typeof markAllReadRemote === "function") {
                    try {
