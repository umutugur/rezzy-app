...
export default function TimeTabs({
  ...
  const handleTab = (k: TimeKey) => {
    if (k === "custom") {
      ...
      t0.setHours(0, 0, 0, 0);
      setStart(t0);
      setEnd(new Date(t0.getTime() + 6 * 24 * 3600 * 1000));
      setActiveIOS(null);
      setRangeOpen(true);
      onChange("custom");
      return;
  ...
  const applyRange = () => {
    ...
    if (s.getTime() > e.getTime()) { const tmp = new Date(s); (s as any) = e; (e as any) = tmp; }
  ...
  const TABS: Array<{ key: TimeKey; label: string; icon: string }> = [
    { key: "all",    label: t("time.all"),    icon: "infinite-outline" },
    { key: "today",  label: t("time.today"),  icon: "today-outline" },
    { key: "week",   label: t("time.week"),   icon: "calendar-outline" },
    { key: "custom", label: t("time.custom"), icon: "calendar-sharp" },
  ];
      ...
      <View style={styles.segmentedControl}>
        {TABS.map((tab) => {
          ...
          return (
            <Pressable
              key={tab.key}
              onPress={() => handleTab(tab.key as TimeKey)}
              style={[styles.tab, active && styles.tabActive]}
            >
              <Ionicons name={tab.icon as any} size={16} color={active ? "#fff" : "#1A1A1A"} />
          ...
          <Pressable style={styles.modalContent} onPress={(e) => e.stopPropagation()}>
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleRow}>
                ...
                <Text style={styles.modalTitle}>{t("time.rangeTitle")}</Text>
              </View>
            ...
            <View style={styles.dateFieldsContainer}>
              <Pressable
                onPress={() => { Platform.OS === "android" ? openAndroidPicker("start") : setActiveIOS("start"); }}
                style={[styles.dateField, activeIOS === "start" && styles.dateFieldActive]}
              >
                <View style={styles.dateFieldHeader}>
                  <Ionicons name="arrow-forward-outline" size={16} color="#7B2C2C" />
                  <Text style={styles.dateFieldLabel}>{t("time.start")}</Text>
                </View>
              ...
              <Pressable
                onPress={() => { Platform.OS === "android" ? openAndroidPicker("end") : setActiveIOS("end"); }}
                style={[styles.dateField, activeIOS === "end" && styles.dateFieldActive]}
              >
                <View style={styles.dateFieldHeader}>
                  <Ionicons name="checkmark-outline" size={16} color="#7B2C2C" />
                  <Text style={styles.dateFieldLabel}>{t("time.end")}</Text>
                </View>
            ...
            <View style={styles.actionsContainer}>
              <Pressable onPress={clearToAll} style={styles.clearButton}>
                ...
                <Text style={styles.clearButtonText}>{t("time.clear")}</Text>
              </Pressable>
              <Pressable onPress={applyRange} style={styles.applyButton}>
                <Ionicons name="checkmark-circle" size={18} color="#fff" />
                <Text style={styles.applyButtonText}>{t("time.apply")}</Text>
              </Pressable>
            </View>
          </Pressable>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  tabText: { color: "#1A1A1A", fontWeight: "700", fontSize: 13 },
  ...
  modalHeader: {
    ...
    marginBottom: 20, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: "#E6E6E6",
  },
  modalTitleRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  modalTitle: { fontSize: 18, fontWeight: "800", color: "#1A1A1A" },
  closeButton: { padding: 4 },
  dateFieldsContainer: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 16 },
  dateField: {
    flex: 1, paddingVertical: 12, paddingHorizontal: 14, borderRadius: 12,
    borderWidth: 2, borderColor: "#E6E6E6", backgroundColor: "#FAFAFA",
  },
  dateFieldActive: { borderColor: "#7B2C2C", backgroundColor: "#FFF5F5" },
  dateFieldHeader: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 6 },
  dateFieldLabel: { fontSize: 12, color: "#666666", fontWeight: "600" },
  dateFieldValue: { fontSize: 16, fontWeight: "700", color: "#1A1A1A" },
  dateFieldDivider: { alignItems: "center", justifyContent: "center" },
  iosPickerContainer: {
    borderWidth: 1, borderColor: "#E6E6E6", borderRadius: 16,
    overflow: "hidden", marginBottom: 16, backgroundColor: "#FAFAFA",
  },
  actionsContainer: { flexDirection: "row", gap: 10 },
  clearButton: {
    flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center",
    paddingVertical: 14, borderRadius: 12, backgroundColor: "#F3F4F6", gap: 6,
  },
  clearButtonText: { fontWeight: "700", color: "#666666", fontSize: 15 },
  applyButton: {
    flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center",
    paddingVertical: 14, borderRadius: 12, backgroundColor: "#7B2C2C", gap: 6,
  },
  applyButtonText: { fontWeight: "700", color: "#fff", fontSize: 15 },
});